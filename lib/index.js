  ...
  return {
    serializer: {
      ...
      processModuleFilter(_module) {
        if (
          filterNodeModules(_module.path) ||
...
const filterNodeModules = modulePath => {
  const checkModaul = path => modulePath.indexOf(path) !== -1;

  if (checkModaul("bpay-card") && !checkModaul("bpay-card/node_modules"))
    return false;
  if (checkModaul("bpay-home") && !checkModaul("bpay-home/node_modules"))
    return false;
  if (checkModaul("node_modules")) return true;
};
